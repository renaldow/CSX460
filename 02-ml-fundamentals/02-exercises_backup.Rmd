...
```{r}
dat <- cars2010 # cars2010, cars2011, cars2012)    # Define 

naive_guess = mean(dat$FE)

set.seed(314)

samp <- dat %>% dplyr::sample_n(10) # you can also do just sample_n(10) since no conflicts with deep lyer package

samp %>% ggplot(aes(x=EngDispl, y=NumCyl) ) + geom_point()
...
#square each term
test1Squared <- (dat$FE - naive_guess)^2

#take mean of entire set
test1mean <- (dat$FE - naive_guess)^2 %>% mean

#take square root of entire set
test1sqrt <- (dat$FE - naive_guess)^2 %>% mean %>% sqrt
...
fit.dat <- lm(FE ~ EngDispl + CarlineClassDesc + EngDispl, data = dat)
