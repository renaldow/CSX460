...
# If using train() method in R, don;t need to split like this since bagging already takes care of ensuring the training data is diversified
...
tc <- trainControl( method="cv", number=10, classProb=TRUE, savePrediction=TRUE )
...
fit.glm <- train(Class ~ ., data = GermanCredit[,-nzv(GermanCredit)], method="glm", family = "binomial", trControl=tc)
...
"ROC Curve"
fit.glm.prediction <- prediction(predictions = fit.glm$pred$Good, labels = fit.glm$pred$obs) 
fit.glm.performance <- performance(fit.glm.prediction, measure = "tpr", x.measure = "fpr") 
plot(fit.glm.performance, main = "ROC Curve for GermanCredit", col="blue", lwd = 3)
abline(a=0, b=1, lwd=2, lty=2)
