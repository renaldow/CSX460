...
?class
gc %>% View 
class(gc$Amount) #class just prints the feature type (RW)
var(gc$Amount)
...
```{r}
fit <- train( Class ~ Amount + Duration, data=gc, trControl=ctrl, method="glm", family="binomial" )
...
```{r}
set.seed(1)
fit <- train( Class ~ ., data=gc, trControl=ctrl, method="glm", family="binomial" )
...
fit.glm.performance.auc <- performance(fit.glm.prediction, measure = "auc")
unlist(fit.glm.performance.auc@y.values)
...
fit.knn <- train( Class ~ ., data=gc, trControl=ctrl, method="knn", tuneGrid=data.frame(k=c(40,50,60,70,80,90)))
...
rpart.plot(fit.rpart, digits = 4, fallen.leaves = TRUE, type = 3, extra=101)
