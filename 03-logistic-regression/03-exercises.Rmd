...
ggplot(data.frame(x = c(-10, 10)), aes(x)) + stat_function(fun = logistic, geom = "line")

?stat_function

xaxis <- seq(from = 0, to = 1, by = .1) 
...
test <- glm(Class ~ ., data=GermanCredit, family = "binomial")

stepAIC( test, scope = ~ 1, direction = "backward")
